...
function alertar(point){
   ...
   // map.setCenter(marker.getPosition());
    var changeShadow = 0;
    var msj_mostrados = new Array();
    msj_mostrados['alertas'] = new Array();
    msj_mostrados['precaucion'] = new Array();
    for (var i = 0; i < vecvertices.length; i++) {
          var tolerancia= radioalerta*0.001 / 111.320;
          var banderaNotif= false;
          var bandAlerta = false;
          var bandPrec = false;
          if(vecvertices[i][6]){
            ...
            if(rsul || resul2){
              ...
              if(msj_mostrados['alertas'].indexOf(i) == -1) {
                  $('#alertas').append(mensaje);
                  msj_mostrados['alertas'].push(i);
              }

              if(msj_mostrados['precaucion'].indexOf(i) == -1) {
                  $('#alertas').append(mensaje);
                  msj_mostrados['precaucion'].push(i);
              }
            //   if(tipomsjOk!="nulo"){
            //       $('#alertas').append("msj");
            //       vecNotif.push(i+tipomsj);
            //       //console.log(i.toString()+tipomsj);
            //   }
              //$('#alertas').append(mensaje);
              $('#alertas').animate({scrollTop: $('#alertas').prop("scrollHeight")}, 200);
            }
        }
